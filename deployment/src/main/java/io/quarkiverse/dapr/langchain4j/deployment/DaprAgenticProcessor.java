...
public class DaprAgenticProcessor {
    ...
    private static final DotName USER_MESSAGE_ANNOTATION = DotName.createSimple("dev.langchain4j.service.UserMessage");

    /** LangChain4j {@code @SystemMessage} annotation. */
    private static final DotName SYSTEM_MESSAGE_ANNOTATION = DotName.createSimple("dev.langchain4j.service.SystemMessage");
     ...
     * <p>
     * <h3>Why a generated decorator?</h3>
     ...
     * This is the same mechanism used by {@link io.quarkiverse.dapr.langchain4j.agent.DaprChatModelDecorator}
     * to wrap the synthetic {@code ChatModel} bean.
     * <p>
     * <h3>What the generated decorator does</h3>
     ...
     * }</pre>
     * <p>
     * Non-{@code @Agent} abstract methods are delegated transparently. Static and default
    ...
    private void generateDecorator(ClassOutput classOutput, IndexView index, ClassInfo agentInterface) {
        ...
        LOG.debugf("Generating CDI decorator %s for @Agent interface %s", decoratorClassName, interfaceName);
        ...
        try (ClassCreator cc = ClassCreator.builder()
            ...
            cc.addAnnotation(Priority.class).addValue("value", Interceptor.Priority.APPLICATION);
            ...
            // CDI decorators are allowed to be "partial" (not implement every inherited method);
            // Arc will delegate un-overridden abstract methods to the next decorator/bean in the
            // chain automatically.
    ...
    private void generateDecoratedAgentMethod(ClassCreator cc, MethodInfo method,
        ...
        if (!isVoid) {
            result = tryBlock.invokeInterfaceMethod(MethodDescriptor.of(method), del, params);
        } else {
        ...
        tryBlock.invokeVirtualMethod(
                MethodDescriptor.ofMethod(AgentRunLifecycleManager.class, "triggerDone", void.class),
                lcmInTry);
        ...
        ResultHandle lcmInCatch = catchBlock.readInstanceField(lcmDesc, catchBlock.getThis());
    ...
    private void generateDelegateMethod(ClassCreator cc, MethodInfo method,
        ...
        if (isVoid) {
            ...
            ResultHandle result = mc.invokeInterfaceMethod(MethodDescriptor.of(method), del, params);
